...
def _money_amount(value):
    if isinstance(value, dict):
        ...
        return _clean_price(amount) if amount not in (None, '') else ''
    return ''


def _extract_sb_shopify_line_items_from_json(value):
    """Return S&B line items from Shopify customer-account GraphQL JSON."""
    items = []
    seen = set()
    ...
    def visit(node):
        if isinstance(node, dict):
            ...
            amount = (
                _money_amount(node.get('currentTotalPrice'))
                or _money_amount(node.get('totalPriceWithDiscounts'))
                or _money_amount(node.get('totalPriceBeforeDiscounts'))
                or _money_amount(node.get('price'))
            )
            if sku and description and amount:
                quantity = _normalize_qty(quantity if quantity not in (None, '') else '1')
                unit_price = _money_amount(node.get('price')) or amount
                try:
                    qty_num = float(quantity)
                    amount_num = float(amount)
                    if qty_num:
                    ...
                    pass
                key = (sku.lower(), description.lower(), quantity, amount)
                if key not in seen:
                    seen.add(key)
                    items.append({
                        'item_number': sku,
...
def _collect_sb_shopify_line_items_from_driver(driver):
    items = []
    try:
    ...
    for item in logs:
        ...
        if 'operation=LineItems' not in url and 'LineItems' not in url:
            continue
        try:
            ...
            parsed = json.loads(response_body.get('body') or '{}')
        except Exception:
            continue
        items.extend(_extract_sb_shopify_line_items_from_json(parsed))

    deduped = []
    seen = set()
    for item in items:
        key = (
            item.get('item_number', '').lower(),
...
def _fetch_rendered_order_context(url, timeout=45):
    """Render a JS-heavy order page in Chrome and return text plus API line items."""
    order_url = str(url or '').strip()
    if not order_url or not SELENIUM_AVAILABLE:
        return {}
    browser = _find_browser_executable()
    if not browser or 'msedge' in os.path.basename(browser).lower():
        return {}

    profile_dir = tempfile.mkdtemp(prefix='invoiceextractor_shopify_')
    driver = None
    try:
        ...
        driver.set_page_load_timeout(max(10, min(int(timeout), 45)))
        driver.get(order_url)

        deadline = time.monotonic() + max(10, min(int(timeout), 35))
        body_text = ''
        order_items_seen_at = None
        while time.monotonic() < deadline:
            try:
                ...
                body_text = ''
            if re.search(r'(?i)\bSKU\s*:', body_text) or re.search(r'(?i)\bLink expired\b', body_text):
                break
            if re.search(r'(?i)\bOrder items\b', body_text):
                order_items_seen_at = order_items_seen_at or time.monotonic()
                if time.monotonic() - order_items_seen_at >= 8:
                    break
            time.sleep(1)

        line_items = []
        for attempt in range(3):
            line_items.extend(_collect_sb_shopify_line_items_from_driver(driver))
            if line_items or attempt == 2:
                break
            time.sleep(2)
        return {
...
def _dedupe_line_items(items):
    ...
    for item in items or []:
        key = (
            str(item.get('item_number') or '').lower(),
...
def _fetch_rendered_order_text(url, timeout=45):
    """Render a JS-heavy order page in Chrome and return document.body.innerText."""
    return _fetch_rendered_order_context(url, timeout=timeout).get('text', '')


def _html_to_visible_text(html):
    text = re.sub(r'(?is)<(script|style|noscript)\b.*?</\1>', '\n', str(html or ''))
    text = re.sub(r'(?i)<br\s*/?>', '\n', text)
    text = re.sub(r'(?i)</(?:p|div|li|tr|td|th|h[1-6]|span|section|article|button)>', '\n', text)
...
def _amount_after_label(lines, label):
    label_re = re.compile(label, re.IGNORECASE)
    amount_re = re.compile(r'\$([\d,]+\.\d{2})')
    for idx, line in enumerate(lines):
        if not label_re.search(line):
            continue
        same_line = amount_re.search(line)
        if same_line:
            return _clean_price(same_line.group(1))
...
def _extract_sb_shopify_order_page_data(page_text):
    """Parse rendered Shopify customer-account order text."""
    lines = [
    ...
    joined = '\n'.join(lines)
    order_match = re.search(r'(?i)\bOrder\s*#\s*(\d+)', joined)
    po_match = re.search(r'(?i)\bPO\s*#?\s*(\d+)', joined)
    due_match = re.search(r'(?i)\bDue\s+([A-Z][a-z]+\s+\d{1,2})(?:,\s*(\d{4}))?', joined)
    ...
    if shipping_method.lower() in {
        ...
        shipping_method = ''
    customer = ''
    ship_block = _extract_block_between_labels(
        joined,
        r'Shipping address',
        [r'Shipping method', r'Billing address', r'Payment', r'Contact information'],
    )
    if ship_block:
        customer = ship_block[0]

    boundary_re = re.compile(
        r'(?i)^(?:Order\s*#|PO\s*#|Expected by|Confirmed|We\'?re|Payment|Pay now|Buy again|'
        r'Contact information|Billing address|Shipping address|Company and location|Shipping method|'
        r'Subtotal|Shipping|Due|Refund policy|Privacy policy|Terms of service|Contact information)$'
    )
    amount_re = re.compile(r'\$([\d,]+\.\d{2})')
    items = []
    for idx, line in enumerate(lines):
        sku_match = re.search(r'(?i)\bSKU\s*:\s*([A-Z0-9._-]+)', line)
        if not sku_match:
            continue
        ...
        desc_parts = []
        j = idx - 1
        while j >= 0 and len(desc_parts) < 5:
            candidate = lines[j].strip()
            if re.fullmatch(r'\d+', candidate):
                quantity = candidate
                break
            if boundary_re.search(candidate) or amount_re.search(candidate) or re.search(r'(?i)\bSKU\s*:', candidate):
                break
            desc_parts.insert(0, candidate)
            j -= 1
        description = ' '.join(desc_parts).strip()
        amount = ''
        same_line_amount = amount_re.search(line)
        if same_line_amount:
            amount = _clean_price(same_line_amount.group(1))
        for next_line in lines[idx + 1:idx + 6]:
            if amount:
                break
            amount_match = amount_re.search(next_line)
            if amount_match:
                amount = _clean_price(amount_match.group(1))
                break
            if re.search(r'(?i)\bSKU\s*:', next_line):
                break
        if not amount:
            for nearby_line in lines[max(0, idx - 3):idx]:
                amount_match = amount_re.search(nearby_line)
                if amount_match:
                    amount = _clean_price(amount_match.group(1))
                    break
    ...
    data = {
        ...
        'customer': customer,
        'due_date': _parse_email_body_date(
            f"{due_match.group(1)}, {due_match.group(2) or datetime.now().year}"
            if due_match
            else ''
        ),
        'shipping_method': shipping_method,
        'subtotal': _amount_after_label(lines, r'^Subtotal\b'),
        'shipping_cost': _amount_after_label(lines, r'^Shipping$'),
        'total': _amount_after_label(lines, r'^Due\b'),
        'line_items': items,
    }
    return {k: v for k, v in data.items() if v}


def _parse_sb_shopify_order_url(url):
    context = _fetch_rendered_order_context(url)
...
def parse_email_invoice(filepath, status_callback=None):
    ...
    if order_url:
        page_data = _parse_sb_shopify_order_url(order_url)
        if page_data and page_data.get('line_items'):
            data['line_items'] = page_data['line_items']
            ...
            cb("  S&B Shopify order page parsed for item/SKU details.")
        else:
            cb("  S&B Shopify order page unavailable; using email body details.", "warning")
